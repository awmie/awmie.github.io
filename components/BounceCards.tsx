...
/* A stack of photos that bounce in (GSAP elastic scale) and, on hover,
   push their siblings apart while the hovered card straightens. */
  ...
  return (
    ...
    >
      {images.map((src, idx) => (
        ...
        >
          <img className="image" src={src} alt={`frame-${idx}`} loading="lazy" />
        </div>
      ))}
    </div>
  )
